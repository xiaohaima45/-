...
//验证是否正确
bool vertify(UserDataType *pUserDataType) {
	unsigned char*usrc = (unsigned char*)pUserDataType->src;
	unsigned char ucontent[32];
	char content2[35];
	MD5_CTX md5;
	MD5Init(&md5);
	MD5Update(&md5, usrc, strlen((char *)usrc));
	MD5Final(&md5, ucontent);
	convertUnCharToStr(content2, ucontent, 16);
	if (strcmp(content2, pUserDataType->dest) == 0) return true;
	return false;
}
...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length(); ++i) {
		...
		if (vertify(&data_seq[i])) printf("\tsuccessful\n");
		else printf("\tfailed\n");
	}
}
